#!/usr/bin/env python3
"""Check for new wasmCloud releases and signal when a new tag should be created."""
...
UPSTREAM_REPO = "wasmCloud/wasmCloud"
...
def validate_assets(release: dict[str, Any]) -> bool:
    """Verify that all expected platform binaries are present in the release."""
...
def main() -> None:
    our_repo = os.environ.get("GITHUB_REPOSITORY", "cataggar/pypi-wash")
    log.info("Checking for new %s releases...", UPSTREAM_REPO)

    release = get_latest_release()
    tag_name = release["tag_name"]  # e.g. "v2.0.2"
    ...
    if not validate_assets(release):
